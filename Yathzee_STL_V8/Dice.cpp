...
void reRoll(map<int, int>& d, list<int>& l)
{
    //char variable
    char choice;
    char pos;
   ...
   for (auto i = l.begin(); i != it; i++)
    {
        if (*i == *i++)
       {
           //i = l.erase(i);// = 7; // need to assign value outside values to check
           l.emplace(i,7);
           l.sort(); //sort function is called to move extra value to end
        }
    }
   for (auto i = l.begin(); i != l.end(); i++)
   {
